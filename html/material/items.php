...
$checklistConfigCsrf = csrfToken('checklist_configuration');
...
<html dir="ltr" lang="en">
  <head>
    ...
    <title>Nuru Real Estate - Checklist Items</title>
    <!-- Favicon icon -->
    <link
      rel="icon"
      type="image/png"
  ...
  <?php include("top-bar.php"); ?>
      ...
      <div class="page-wrapper">
        ...
        <div class="container-fluid">
          ...
          <!-- Start Page Content -->
          <!-- -------------------------------------------------------------- -->
		<?php define('NURU_ITEM_FORM_INCLUDE', true); include("add_items.php"); ?>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const stageSelect = form.querySelector('[name="stage_id"]');

  submitBtn.disabled = true;

  fetch('./config/get_stages.php')
    .then(res => res.json())
    .then(data => {
      ...
      submitBtn.disabled = false; // ✅ NOW it makes sense
    })
    .catch(() => {
      stageSelect.innerHTML = '<option value="">Failed to load</option>';
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    fetch('./config/save_checklist_item.php', {
      method: 'POST',
    ...
    .then(data => {
      ...
      message.textContent = data.message || 'Checklist item added successfully.';
      form.reset();

    })
    .catch(err => {
      message.className = 'alert alert-danger mt-3';
      message.textContent = err.message || 'Unable to add the checklist item.';
    })
    .finally(() => {
      isSubmitting = false;
