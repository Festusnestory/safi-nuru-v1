...
/** @var array $sellers */
/** @var string $words */
/** @var string $sellerManagementCsrf */
/** @var string $baseUrl */
?>
<!DOCTYPE html>
<html dir="ltr" lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="csrf-token" content="<?= htmlspecialchars($sellerManagementCsrf, ENT_QUOTES, 'UTF-8') ?>" />
  ...
  <body>
	...
	?>
      ...
      <!-- Page wrapper  -->
      <!-- ============================================================== -->
		<?php define('NURU_SELLER_LIST_INCLUDE', true); require __DIR__ . '/_table.php'; ?>
      <!-- ============================================================== -->
      <!-- End Page wrapper  -->
      <!-- ============================================================== -->
    </div>
...
<script>
const nuruSellerListBaseUrl = <?= json_encode($baseUrl) ?>;
...
document.addEventListener('click', async event => {
    const button = event.target.closest('.seller-decision');
    if (!button) return;
    ...
    document.querySelectorAll(`.seller-decision[data-id="${button.dataset.id}"]`).forEach(item => item.disabled = true);
    try {
      const response = await fetch(nuruSellerListBaseUrl + '/admin/review-seller-application', {
        method: 'POST',
        headers: {'Content-Type': 'application/json', 'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').content},
        body: JSON.stringify({application_id: button.dataset.id, decision, reason})
      });
      ...
      document.querySelectorAll(`.seller-decision[data-id="${button.dataset.id}"]`).forEach(item => item.disabled = false);
    }
});
</script>
