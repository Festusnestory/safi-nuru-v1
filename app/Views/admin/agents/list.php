...
<html dir="ltr" lang="en">
  ...
  <body>
    ...
    ?>
      ...
      <!-- Page wrapper  -->
      <!-- ============================================================== -->
<?php define('NURU_AGENT_LIST_INCLUDE', true); require __DIR__ . '/_table.php'; ?>
      <!-- ============================================================== -->
      <!-- End Page wrapper  -->
      <!-- ============================================================== -->
    </div>
...
const agentsTable = $("#agents_table");
if (agentsTable.length && $.fn.DataTable) {
  agentsTable.DataTable({
    dom: "Bfrtip",
    buttons: ["copy", "csv", "excel", "pdf", "print"],
    language: { emptyTable: "No agents found." },
  });
}
$(
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.agent-status').forEach(select => {
        ...
        select.addEventListener('change', function () {
            ...
            const badge = document.getElementById(`status-badge-${agentId}`);
            const loader = document.getElementById(`status-loader-${agentId}`);
            const globalLoader = document.getElementById('global-status-loader');

            // SAFETY CHECKS
            if (!badge || !loader) {
                console.error('Status elements missing for agent ID:', agentId);
                return;
            }
            ...
            fetch(nuruAgentListBaseUrl + '/admin/update-agent-status', {
                method: 'POST',
    ...
    // Pending agent application decisions
    document.querySelectorAll('.application-decision').forEach(btn => {
        btn.addEventListener('click', function () {
            const decision = this.dataset.decision;
            const applicationId = this.dataset.id;

            if (decision === 'reject' && !window.nuruConfirmClick(this, 'Confirm rejection')) return;

            const actionButtons = this.closest('td').querySelectorAll('.application-decision');
            actionButtons.forEach(button => {
                button.disabled = true;
                button.setAttribute('aria-disabled', 'true');
            });
            const originalText = this.textContent;
            this.textContent = decision === 'approve' ? 'Approving…' : 'Rejecting…';

            fetch(nuruAgentListBaseUrl + '/admin/approve-agent-application', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                },
                body: JSON.stringify({ application_id: applicationId, decision: decision })
            })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    document.querySelectorAll('.delete-agent').forEach(btn => {
        btn.addEventListener('click', function () {
            if (!window.nuruConfirmClick(this, 'Confirm deactivation')) return;

            const agentId = this.dataset.id;

            fetch(nuruAgentListBaseUrl + '/admin/delete-agent', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
