        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div id="taskActionMessage" role="status" aria-live="polite"></div>
                        <div class="table-responsive">
							...
							<tr class="<?= $t['is_overdue'] ? 'table-warning' : '' ?>">
								...
								<td><span class="badge <?= $t['status'] === 'on_hold' ? 'bg-warning text-dark' : 'bg-secondary' ?>"><?= $t['status'] === 'on_hold' ? 'On Hold' : 'Assigned' ?></span></td>
								<!-- TASK -->
								...
								<!-- TRANSACTION -->
								<td>N$ <?= number_format((float)$t['selling_price'], 2) ?></td>
								<td>
								  <?= htmlspecialchars(preg_split('/\s+/', trim($t['buyer_name'] ?? ''))[0] ?? '—') ?>
								</td>
								...
								<td class="text-nowrap">
								  ...
								  data-allocation-id="<?= $t['task_id'] ?>"
								  data-action="in_progress"
								>
								  <?= $t['status'] === 'on_hold' ? 'Resume' : 'Accept' ?>
								</button>

								<?php if ($t['status'] !== 'on_hold'): ?>
								<button
								  class="btn btn-warning task-action"
								  data-allocation-id="<?= $t['task_id'] ?>"
								  data-action="hold"
								>
								  Hold
								...
								<button
								  class="btn btn-danger task-action"
								  data-allocation-id="<?= $t['task_id'] ?>"
								  data-action="cancel"
								>
								  Cancel
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.reassign-agent').forEach(select => {
        select.addEventListener('change', function () {
            ...
            fetch(<?= json_encode(\portalPath('config/reassign_task.php')) ?>, {
                method: 'POST',
...
document.addEventListener('click', function (e) {
    ...
    const message = document.getElementById('taskActionMessage');

    if (!allocationId || !action) {
        message.className = 'alert alert-danger';
        message.textContent = 'Invalid task action.';
        return;
    }
    ...
    if (btn.dataset.confirmed !== 'true') {
        ...
        btn.textContent = `Confirm ${action.replace('_', ' ')}`;
        message.className = 'alert alert-warning';
        message.textContent = 'Click the same button again to confirm.';
        window.setTimeout(() => {
            if (btn.dataset.confirmed === 'true') {
    ...
    btn.disabled = true;
    fetch(<?= json_encode(\portalPath('config/agent_update_task_status.php')) ?>, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
            allocation_id: allocationId,
            action: action,
            csrf_token: <?= json_encode($taskStatusCsrf) ?>
        })
    })
    .then(async res => ({ ok: res.ok, data: await res.json() }))
    .then(({ ok, data }) => {
        if (!ok || !data.success) throw new Error(data.message || 'The task could not be updated.');
    ...
    .catch(error => {
        btn.disabled = false;
        btn.dataset.confirmed = 'false';
        btn.textContent = btn.dataset.originalText || 'Retry';
        message.className = 'alert alert-danger';
