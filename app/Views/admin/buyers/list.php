  ...
  <body>
    ...
    <div id="main-wrapper">
     ...
     <?php require NURU_MATERIAL . '/top-bar.php'; ?>
      ...
      <!-- Page wrapper  -->
      <!-- ============================================================== -->
		<?php define('NURU_BUYER_LIST_INCLUDE', true); require NURU_MATERIAL . '/buyerlist_table.php'; ?>
      <!-- ============================================================== -->
      <!-- End Page wrapper  -->
      <!-- ============================================================== -->
    </div>
...
function deleteBuyer(buyerId, button) {
    if (!button || !window.nuruConfirmClick(button, 'Confirm rejection')) return;

    fetch('<?= $baseUrl ?>/html/material/config/delete_buyer.php', {
        method: 'POST',
        headers: {
...
document.querySelectorAll('.buyer-delete').forEach(button => {
    button.addEventListener('click', function () {
        deleteBuyer(this.dataset.id, this);
    });
});

document.querySelectorAll('.buyer-approve').forEach(btn => {
    btn.addEventListener('click', function () {
        const buyerId = this.dataset.id;
        fetch('<?= $baseUrl ?>/html/material/config/approve_buyer.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .catch(() => alert('Failed to approve buyer. Please try again.'));
    });
});
