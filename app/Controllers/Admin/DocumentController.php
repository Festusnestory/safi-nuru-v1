...
/**
 * Streams uploaded buyer/seller/agent documents
 * (html/material/view_document.php). Uses sessionHasAuthoritativeRole()
 * directly (like the original) rather than requireRole()/$this->requireRole()
 * so unauthorized/unauthenticated requests get a plain 403 response instead
 * of a login-page redirect - this is a raw file endpoint, not an HTML page.
 */
final class DocumentController extends Controller
{
    public function view(): void
    {
        ...
        $uploadsRoot = realpath(\NURU_MATERIAL . '/uploads');
        ...
        if (Auth::currentRole() === 'agent_coordinator') {
            ...
            if ($agentId !== null && str_starts_with($requestedPath, 'uploads/buyers/')) {
                $buyerStmt = $this->pdo->prepare('SELECT buyer_id FROM buyer_documents WHERE file_path = :path LIMIT 1');
                $buyerStmt->execute([':path' => $requestedPath]);
                $buyerId = $buyerStmt->fetchColumn();
                if ($buyerId !== false) {
                    $ownsBuyer = $this->pdo->prepare("SELECT 1 FROM buyers b
            ...
            if ($agentId !== null && str_starts_with($requestedPath, 'uploads/seller/')) {
                ...
                foreach ($sellerFileSources as [$table, $column]) {
                    $fileStmt = $this->pdo->prepare("SELECT application_id FROM {$table} WHERE {$column} = :path LIMIT 1");
                ...
                if ($applicationId !== null) {
                    $ownsSeller = $this->pdo->prepare("SELECT 1 FROM seller_applications sa
