...
/**
 * Internal staff-facing "quick consult" workflow: log a walk-in / bank
 * pre-approved buyer consultation and list the ones this consultant (or
 * everyone, for admin/manager) has logged. Ported from
 * consulting_agent_form.php, admin_consulting_agent_processor.php, and
 * consultant_list.php - kept as its own controller (separate from
 * AgentController) since it operates on the buyers table, not agents.
 */
final class ConsultantController extends \App\Core\Controller
{
    public function form(): void
    {
        ...
        $this->render('admin.consultants.form', [
            'csrfToken' => Auth::csrfToken('consulting_agent_form'),
    ...
    {
        Bootstrap::requireSession();
        $this->requireRole(['admin', 'manager', 'agent_consultant']);
        require_once \NURU_MATERIAL . '/config/id_tokens.php';

        $loadedBy = Auth::currentRole() === 'agent_consultant' ? (int) $_SESSION['user_id'] : null;

        $this->render('admin.consultants.list', [
            'buyers' => (new Consultant($this->pdo))->list($loadedBy),
            'baseUrl' => Router::basePath(),
        ]);
    }

    /**
     * Consulting agent form submission for pre-approved bank buyers. Ported
     * near-verbatim from admin_consulting_agent_processor.php - same
     * validation, same buyers/buyer_preferred_areas writes, same AJAX vs.
     * redirect response branching.
     */
    public function store(): void
    {
        ...
        try {
            ...
            $this->validateConsultation($pdo, $data);

            $this->consultantStoreDebugLog('Consulting agent form submitted');

            $pdo->beginTransaction();

            $applicationNumber = $this->generateApplicationNumber($pdo);
            ...
            $this->consultantStoreDebugLog('Buyer created from Consulting Agent form with ID: ' . $buyerId);

            // 2. Save preferred areas to buyer_preferred_areas table
            $this->savePreferredAreas($pdo, $buyerId, $data);

            $pdo->commit();
            unset($_SESSION['_csrf_tokens']['consulting_agent_form']);

            $this->consultantStoreDebugLog('Consulting Agent buyer application submitted successfully: ' . $applicationNumber);
            ...
            $this->consultantStoreDebugLog('ERROR: ' . $e->getMessage(), 'ERROR');
    ...
    private function consultantStoreDebugLog(string $msg, string $level = 'INFO'): void
    {
    ...
    private function generateApplicationNumber(\PDO $pdo): string
    {
    ...
    /** Save preferred areas to buyer_preferred_areas table */
    private function savePreferredAreas(\PDO $pdo, int $buyerId, array $data): void
    {
        if (!empty($data['preferredRegion']) && is_array($data['preferredRegion'])) {
            ...
            $this->consultantStoreDebugLog('Saved ' . $count . ' preferred areas for buyer ID: ' . $buyerId);
        }
    }

    private function validateConsultation(\PDO $pdo, array &$data): void
    {
...
/** Matches admin_consulting_agent_processor.php's local ConsultationValidationException. */
