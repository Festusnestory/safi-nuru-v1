...
if ($id) {
    $property = publicPropertyById($pdo, $id);
    if ($property) {
        $stmt = $pdo->prepare('SELECT file_path, alt_text FROM seller_property_images WHERE propertyId = :id ORDER BY is_primary DESC, image_order, id');
        $stmt->execute([':id' => $id]); $images = $stmt->fetchAll();
    }
}
if (!$property) { http_response_code(404); renderHeader('Property not found', 'The requested property is no longer available.', 'properties'); ?>
<main id="main"><section class="page-hero"><p class="eyebrow">PROPERTY UNAVAILABLE</p><h1>This listing has<br>moved on.</h1><p>Browse our current collection or speak with us about similar opportunities.</p><a class="button button-dark" href="properties.php">View properties</a></section></main><?php renderFooter(); exit; }
...
<main id="main">
  ...
  <section class="detail-gallery <?= count($images) > 1 ? 'multi' : '' ?>">
    <?php $gallery = $images ?: [['file_path' => $property['primary_image'], 'alt_text' => null]]; foreach (array_slice($gallery, 0, 3) as $image): ?><img src="<?= e(publicPropertyImage($image['file_path'])) ?>" alt="<?= e($image['alt_text'] ?: $property['property_detail_type'] . ' in ' . $property['property_town']) ?>"><?php endforeach; ?>
  </section>
